...
class Agenda:
  def __init__(self, id, data, confirmado, nome_cliente, desc_servico):
    self.__id = id
    self.__data = data
    self.__confirmado = confirmado
    self.__nome_cliente = nome_cliente
    self.__desc_servico = desc_servico

  def get_id(self): return self.__id
  def get_data(self): return self.__data
  def get_confirmado(self): return self.__confirmado
  def get_id_cliente(self): return self.__nome_cliente
  def get_id_servico(self): return self.__desc_servico
  ...
  def set_desc_servico(self, desc_servico): self.__desc_servico = desc_servico

  def __eq__(self, x):
    if self.__id == x.__id and self.__data == x.__data and self.__confirmado == x.__confirmado and self.__nome_cliente == x.__nome_cliente and self.__desc_servico == x.__desc_servico:
      return True
    return False

  def __str__(self):
    return f"{self.__id} - {self.__data.strftime('%d/%m/%Y %H:%M')} - {self.__confirmado} - {self.__nome_cliente} - {self.__desc_servico}"
  ...
  def to_json(self):
    return {
      ...
      'desc_servico': self.__desc_servico}
...
class NAgenda:
  ...
  def abrir(cls):
    cls.__agendas = []
    try:
      with open("ListaPoo9C/agendas.json", mode="r") as arquivo:
        agendas_json = json.load(arquivo)
        for obj in agendas_json:
          aux = Agenda(
            obj["id"],
            datetime.datetime.strptime(obj["data"], "%d/%m/%Y %H:%M"),
            obj["confirmado"], obj["nome_cliente"], obj["desc_servico"])
  ...
  @classmethod
  def salvar(cls):
    with open("ListaPoo9C/agendas.json", mode="w") as arquivo:
